  ...
  <div class="app-container contentBoxGlobal" ref="contentBoxRef">
    <div ref="searchBoxRef">
      <el-card shadow="never" class="search-wrapper">
        <el-form ref="searchFormRef" :inline="true" :model="searchData">
          ...
          <el-form-item prop="type" label="所属出/入库">
            <el-select v-model="searchData.type" placeholder="请选择" style="width: 200px" clearable>
    ...
    <el-card shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" :max-height="height" v-loading="loading" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="name" label="出入库类型名称" />
          <el-table-column label="所属出/入库">
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-position="left" label-width="auto">
        <el-form-item prop="name" label="出入库类型名称">
          <el-input v-model="formData.name" placeholder="请输入出入库类型名称" />
        </el-form-item>
        <el-form-item prop="type" label="所属出/入库">
          <el-radio-group v-model="formData.type">
            <el-radio v-for="(v, i) in libraryType" :value="v.value">{{ v.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
    </global-dialog>
...
import { delWarehouseList, editWarehouseList, getWarehouseList } from "@/api/system"
...
} = useTable(contentBoxRef, searchBoxRef, getWarehouseList, delWarehouseList, paginationData, searchData)
const resetSearch = () => {
...
const formRules: FormRules = {
  name: [{ required: true, trigger: "blur", message: "请输入出入库类型名称" }],
  type: [{ required: true, trigger: "change", message: "请选择所属出入库" }]
}
const resetForm = () => {
...
const addAndUpdateFun = () => {
  formRef.value?.validate((valid: boolean, fields) => {
    ...
    editWarehouseList(formData.value)
      .then(() => {
